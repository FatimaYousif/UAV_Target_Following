...
#     rosbag_files = ["/home/fatima/bytetrack_sim1.bag"]#, "data/2024-04-16/EXP6.bag", "data/2024-04-16/EXP9.bag", "data/2024-04-16/EXP12.bag"]
#     x_arrays_all = []
#     y_arrays_all = []
...
def read_rosbag(rosbag_file):
    ...
    # Skip initial frames before tracking starts
    init = 0
    for i, lost in enumerate(target_lost_values):
        if lost == 1:
            break
        init += 1

    # Slice arrays from first detection
    center_x = center_x[init:]
    center_y = center_y[init:]

    x_arrays.extend(center_x)
    y_arrays.extend(center_y)
    return [x_arrays], [y_arrays]  # Return as nested lists for compatibility
...
def plot_probability_distribution(x_arrays, y_arrays):
    ...
    xi, yi = np.mgrid[
        (xmin - padding_x):(xmax + padding_x):300j,
        (ymin - padding_y):(ymax + padding_y):300j
    ]
    ...
    # Normalize to percentage for consistency with reference figure
    zi = zi / zi.sum() * 100
    ...
    cbar = plt.colorbar(contour, label='Density distribution (%)')
    plt.xlabel('X Position (pixel)')
    plt.ylabel('Y Position (pixel)')
    plt.title("BoT-SORT Tracking")
    plt.grid(True)
    plt.show()

if __name__ == "__main__":
    rosbag_files = ["/home/fatima/botsort_sim2.bag"]
    x_arrays_all = []
    y_arrays_all = []

    for rosbag_file in rosbag_files:
        x_arrays, y_arrays = read_rosbag(rosbag_file)
        print(f"Sample X: {x_arrays[0][:5]}")
        print(f"Sample Y: {y_arrays[0][:5]}")
        print(f"Total Points: {len(x_arrays[0])}")
        x_arrays_all.extend(x_arrays)
        y_arrays_all.extend(y_arrays)
