...
axes[0, 0].plot(timestamps[init:], real_distances1[init:], label='Real Distance')
axes[0, 0].plot(timestamps[init:], estimated_distances[init:], label='Estimated Distance ')
...
print("TOTAL DISTANCE TARGET", total_distance_target)
sum=0
for i in target_lost_values:
    if(target_lost_values[i] == False):
        sum+=1
print("TARGET LOST # of times", sum)
print("PERCENTAGE TARGET FOUND", percentage)
# print("PERCENTAGE DEPTH VALUES", percentage_depth)
print("AVERAGE ESTIMATION ERROR", average_error)
